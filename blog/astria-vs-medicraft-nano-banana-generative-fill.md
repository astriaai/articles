...
image: /img/covers/astria-vs-medicraft-nano-banana-generative-fill.webp
authors: [astria]
tags: [comparisons, photoshop]
keywords:
  - nano banana generative fill plugin
  - medicraft nano banana photoshop
  - astria vs medicraft
  - nano banana photoshop plugin
  - aescripts nano banana
---
...
export const faqSchema = {
  ...
  mainEntity: [
    ...
    {
      "@type": "Question",
      name: "Can I use my Gemini subscription with the Medicraft or Astria plugin?",
      acceptedAnswer: {
        "@type": "Answer",
        text: "No. Google only authorises external apps through API keys, so a consumer Gemini subscription cannot power any Photoshop plugin. Medicraft needs a Google AI Studio key backed by Google billing for anything beyond the original Nano Banana. Astria accepts the same Google key, or an Astria key that covers every model in the panel.",
      },
    },
    {
      "@type": "Question",
      name: "Does the Astria plugin support bring-your-own-key like Medicraft?",
      acceptedAnswer: {
        "@type": "Answer",
        text: "Yes. Paste a Google AI Studio key into the Astria panel and Nano Banana calls run on your own Google quota, billed by Google. Astria also offers a hosted pay-as-you-go path through one Astria key covering GPT Image 2, Seedream and Flux as well as Gemini, with quota, authentication and retries handled for you.",
      },
    },
    {
      "@type": "Question",
      name: "Which plugin works on older versions of Photoshop?",
      acceptedAnswer: {
        "@type": "Answer",
        text: "Medicraft. Adobe Exchange lists it for Photoshop 22.2 or higher and the aescripts description says 24.2 (2023) and above, with Actions recording from version 25. The Astria plugin's CCX build requires Photoshop 2025 (version 26 or later) on macOS or Windows.",
      },
    },
    {
      "@type": "Question",
      name: "Is Astria better than Medicraft Nano Banana Generative Fill?",
      acceptedAnswer: {
        "@type": "Answer",
        text: "For a retoucher, photographer or ecommerce studio on Photoshop 2025 or later, yes: six hosted models instead of one model line, a billing path with no Google Cloud billing account, Smart Objects at full returned resolution, zero-day model arrivals and a production route for the rest of the catalogue. Medicraft remains reasonable for a Gemini-only user on Photoshop 2023 or 2024 who already runs Google billing.",
      },
    },
  ],
};
...
Reviewed 13 September 2026. Medicraft's returned resolution and layer type are reported from its public material; no side-by-side output test was run.
...
Inside the document, Medicraft has built useful things: references from the active layer or a file, output as a new layer or into the original, an edge-feather helper, an editable prompt library with history, and Actions recording on Photoshop 25 and later. The version floor is low: Adobe Exchange lists Photoshop 22.2 or higher and the aescripts description says 24.2 (2023) and above. Support runs through a ticket portal, and the readme's advice for "Internal API Error" is to click Generate again.

## The Astria Nano Banana plugin for Photoshop: six hosted models, two billing paths
...
Medicraft's evidence is its two listings and a version history from 28 September 2025 to 2.3.0 on 2 July 2026. Adobe Exchange reviews are mixed; the critical ones cluster around key and quota errors rather than image quality, which matches the vendor's own notes about connecting a payment method. In our September 2026 search checks the aescripts page sat at the top of results for the plugin query.

Astria's evidence is first-party: the landing page reports 16,000 sales and 187 ratings, and shows seven production reels covering fashion edits with references, bridal retouching, sketch and CAD to photo, product design, real estate, hair and clothing swaps, and packaging. Both vendors' numbers are adoption signals, not quality benchmarks.

## One campaign hero, then forty SKUs: the same job in both plugins

A footwear brand sends a retoucher one campaign hero. The shoes need swapping for the new colourway from a product reference, the background needs extending for a 4:5 crop, and a phone in the model's hand has to go. Then the brand mentions forty more SKUs.
...
Medicraft's changelog shows its cadence: Nano Banana 2 on 27 February 2026, Nano Banana 2 Lite on 2 July 2026, each a plugin release you download. The Astria panel reads its roster from the platform, so a new model is a server-side addition, not a plugin update.
...
Credit where it is due, once. Medicraft's edge-feather helper, editable prompt library, in-place output option and Actions recording are real conveniences the Astria plugin does not advertise. Its Photoshop floor is also lower, at 22.2 on Exchange or 24.2 per aescripts, against Astria's Photoshop 2025 requirement. If you are pinned to Photoshop 2023 or 2024, Medicraft is the panel you can actually run.
...
- You are on Photoshop 2023 or 2024 and cannot move to Photoshop 2025.
- You already run Google billing and want Google's list price with no intermediary.
- Gemini is the only model line you will ever send a selection to.
- You want an edge-feather helper, a prompt library and Actions recording in the panel.

## Choose Astria if...

- You want GPT Image 2 with mask, Nano Banana, Seedream 5 and Flux 2 Pro in one dropdown, chosen per task.
- You would rather top up one balance than open and monitor Google billing, or you want both options.
- Results must arrive as Smart Objects at full returned resolution.
- You want new models on launch day, and a route from the hero image to the rest of the catalogue.
...
Run Medicraft's free trial and the Astria plugin on the same PSD: one brushed selection, one reference-driven swap, one canvas extension. Record each result's returned pixel dimensions, whether it arrives as a Smart Object or flattened pixels, and the steps between selection and layer. Time the setup honestly, including Google billing for Medicraft. Then run the same swap with GPT Image 2 and Seedream 5 in the Astria panel, the comparison Medicraft cannot join. Finally, price ten realistic edits on each path from Google's and Astria's pricing pages rather than either vendor's headline.

If you are also weighing Photoshop's built-in models, read [Astria vs Photoshop Generative Fill](./astria-plugin-vs-photoshop-generative-fill.md). If bring-your-own-key depth is the priority, [Astria vs Creator MAX](./astria-vs-creator-max-photoshop-plugin.md) covers the feature-richest BYOK panel. The full shortlist is in [best Nano Banana Photoshop plugins](./best-nano-banana-photoshop-plugins.md).
...
No. Google only authorises external apps through API keys, so a consumer Gemini subscription cannot power any Photoshop plugin. Medicraft needs a Google AI Studio key backed by Google billing for anything beyond the original Nano Banana. Astria accepts the same Google key, or an Astria key that covers every model in the panel.

### Does the Astria plugin support bring-your-own-key like Medicraft?

Yes. Paste a Google AI Studio key into the Astria panel and Nano Banana calls run on your own Google quota, billed by Google. Astria also offers a hosted pay-as-you-go path through one Astria key covering GPT Image 2, Seedream and Flux as well as Gemini, with quota, authentication and retries handled for you.

### Which plugin works on older versions of Photoshop?

Medicraft. Adobe Exchange lists it for Photoshop 22.2 or higher and the aescripts description says 24.2 (2023) and above, with Actions recording from version 25. The Astria plugin's CCX build requires Photoshop 2025 (version 26 or later) on macOS or Windows.

### Is Astria better than Medicraft Nano Banana Generative Fill?

For a retoucher, photographer or ecommerce studio on Photoshop 2025 or later, yes: six hosted models instead of one model line, a billing path with no Google Cloud billing account, Smart Objects at full returned resolution, zero-day model arrivals and a production route for the rest of the catalogue. Medicraft remains reasonable for a Gemini-only user on Photoshop 2023 or 2024 who already runs Google billing.

## Sources and methodology

This article compares public product information reviewed on 13 September 2026. Medicraft sources: the [aescripts product page](https://aescripts.com/nano-banana-generative-fill-for-photoshop/) for price, licence types, free trial, the Google trial-credit warning, the Photoshop 24.2 floor and the version history (read in a browser; automated fetches are refused); the [Adobe Exchange listing](https://exchange.adobe.com/apps/cc/7ca530ba/nano-banana-generative-fill-for-photoshop) for the Nano Banana Pro statement, Actions recording on Photoshop 25+, the Photoshop 22.2 compatibility line, package size and publication date; and the vendor's [setup readme](https://unwarp.medicalworkseg.com/NanoBanana_readme.pdf) for key, billing and error-handling steps. The trial-credit warning is dated 26 March without a year; 2026 follows from the listing's timeline. Google facts come from the [Gemini API pricing page](https://ai.google.dev/gemini-api/docs/pricing), updated 11 September 2026. Astria sources: the [plugin landing page](https://www.astria.ai/nano-banana-photoshop) for models, workflow, billing, changelog, requirements, refund policy and first-party traction; the [pricing page](https://www.astria.ai/pricing); and the live platform catalogue on 13 September 2026.

No competitor product was purchased or run for this article; returned-resolution and layer-type behaviour of Medicraft Nano Banana Generative Fill is reported from its public material only. Sales and rating counts are first-party claims. Prices, model lists and Google's API policies change often; confirm both vendors' current pages before buying.
